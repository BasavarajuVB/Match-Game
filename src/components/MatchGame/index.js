...
class MatchGame extends Component {
  ...
  getActiveTabImages = searchedImages => {
    const {activeTabId} = this.state
    const filteredImages = searchedImages.filter(
      eachSearchedImage => eachSearchedImage.category === activeTabId,
    )
  ...
  setScore = id => {
    const {renderImageId} = this.state
    if (id === imagesList[renderImageId].id) {
      this.setState(prevState => ({score: prevState.score + 1}))
      this.setState({
        renderImageId: Math.floor(Math.random() * 30),
      })
    } else {
      this.setState(prevState => ({
        isTimerRunning: !prevState.isTimerRunning,
  ...
  render() {
    ...
    const filteredImages = this.getActiveTabImages(imagesList)
    const renderMatchGame = isTimerRunning ? (
      <li>
        <div className="container">
          <img
            src={imagesList[renderImageId].imageUrl}
            alt="match"
            className="match-img"
          />
    ...
    return (
      <div className="app-container">
        ...
        <ul className="render-section">{renderMatchGame}</ul>
      </div>
    )
  }
}
